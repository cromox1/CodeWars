...
def phone(strng, num):
    phoneline = [s for s in strng.split("\n") if num in s]
    if len(phoneline) == 1:
        phoneline = phoneline[0]
    elif len(phoneline) > 1:
        return "Error => Too many people: " + num
    else:
        return "Error => Not found: " + num
    phonenumber = phoneline.split("+")[1].split(" ")[0].replace(";","").replace("!","").replace(",","")
    name = phoneline.split("<")[1].split(">")[0]
    address = phoneline.replace("<" + name + ">", "").replace("+" + phonenumber, "").replace("_", " ")
    newaddress = buangsimbol(address).replace("  ", " ").lstrip().rstrip()
    return "Phone => " + phonenumber + ", Name => " + name + ", Address => " + newaddress

def buangsimbol(strline):
    simbol = ["/", ";", "$", "*", "!", "?", ",", ":", ]
    for str in simbol:
        strline = strline.replace(str, "")
    return strline
